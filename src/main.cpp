...
int main() {
  ...
  while (getline(in_map_, line)) {
  	...
  	iss >> y;
  	iss >> s;
  	iss >> d_x; // normal component to the waypoints, same for d_y
  	iss >> d_y;
      ...
      if (s != "") {
        ...
        if (event == "telemetry"){
          // j[1] is the data JSON object
            ...
            // either we will reference the starting point as where hte car is or at the previous paths end point
            double ref_x = car_x;
            double ref_y = car_y;
            ...
            {
              //shift car reference angle to 0 degrees: the car's local coordinate
